...
public class AutomaticLoggingImpl {
  ...
  public Object logExecution(ProceedingJoinPoint joinPoint) throws Throwable {
    ...
    // The names and values of the target parameters are captured, as well as the values in the
    // annotation.
    ...
    // Iterate between the parameters if the option has not been disabled in the annotation and add
    // them to the String
  ...
  private void addParametersToLog(
      AutomaticLogging annotation,
      AtomicReference<String> logContent,
      String[] parameterNames,
      Object[] parameterValues) {
    if (annotation.logCollectionsSizeOnly()) {
      // If the parameters are Collections and the logCollectionsSizeOnly() parameter of the
      // Annotation is set to True, only the size will be reported and not the content.
      for (int i = 0; i < parameterNames.length; i++) {
        logContent.set(logContent.get() + parameterNames[i]);
        boolean collectionParam =
            switchType(
                parameterValues[i],
                caze(
                    Collection.class,
                    c -> logContent.set(logContent.get() + " size : " + c.size() + " ")),
                caze(
                    Map.class,
                    a -> logContent.set(logContent.get() + " size : " + a.size() + " ")));
        if (!collectionParam) {
          logContent.set(
              logContent.get()
                  + ": "
                  + (parameterValues[i] != null ? parameterValues[i].toString() : "null")
                  + " ");
        }
      }
    } else {
      for (int i = 0; i < parameterNames.length; i++) {
        logContent.set(
            logContent.get() + parameterNames[i] + ": " + parameterValues[i].toString() + " ");
      }
    }
  }
}
