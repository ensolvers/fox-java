...
public class AutomaticLoggingImpl {
  ...
  public Object logExecution(ProceedingJoinPoint joinPoint) throws Throwable {
    ...
    AtomicReference<String> logContent = new AtomicReference<String>(
            (!annotation.logSuffix().equals("") ?
                          "[" + annotation.logSuffix() + "] ": "") +
                        "["+ className + "] " + "[" + methodSignature.getName() + "] " +
    ...
    try {
      ...
      // When it is finished, we calculate how long it took and the message is updated to be logged again if TimeElapsedLogging is activated */
      ...
      logger.error((!annotation.logSuffix().equals("") ? "[" + annotation.logSuffix() + "] ": "") + "["+ className + "] " + "[" + methodSignature.getName() + "] Error: " + error);
      throw error;
    }
