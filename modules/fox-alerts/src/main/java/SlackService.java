...
public class SlackService {
   ...
   *
   * @param token bot token that will be used to send messages - an app must be created and
   *     installed an da bot token should be generated and provided
   * @param defaultChannel for methods that do not require a channel, the channel id that will be
  ...
  public ChatPostMessageResponse sendMessage(String channelId, String message) throws Exception {
    return this.instance
        .methods(this.token)
        .chatPostMessage(req -> req.channel(channelId).mrkdwn(true).text(message));
  }
   ...
   * @throws Exception if an error or unexpected situation occurs when trying to send the message
   */
  public ChatPostMessageResponse sendMessageWithColor(String message, String color)
      throws Exception {
    return this.instance
        .methods(this.token)
        .chatPostMessage(
            req ->
                req.channel(this.defaultChannel)
                    .attachments(
                        Collections.singletonList(
                            Attachment.builder().color(color).text(message).build())));
  }
