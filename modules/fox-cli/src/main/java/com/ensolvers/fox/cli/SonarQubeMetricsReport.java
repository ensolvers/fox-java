...
/**
 * Command-line application that allows to generate and send SonarQube reports
 * to a Slack channel
 *
...
public class SonarQubeMetricsReport implements Callable<Integer> {
    ...
    @CommandLine.Option(names = {
            "--sonar-component" }, description = "SonarQube component from which the metrics will be fetched", required = true)
    private String sonarComponent;

    @CommandLine.Option(names = {
            "--slack-bot-token" }, description = "Slack bot token that will be used to publish the results", required = true)
    private String slackBotToken;

    @CommandLine.Option(names = {
            "--slack-channel" }, description = "Slack channel ID in which the results will be published", required = true)
    ...
    public Integer call() throws Exception {
        ...
        try {
            ...
            SonarQubeMetricHistoryResponse history = sonarQubeService.getMetricHistory(this.sonarComponent, metric,
            ...
            if (!history.getMeasures().isEmpty()) {
                ...
                message = message + history.getMeasures().get(0).getHistory().stream().map(m -> m.getDate() + ": " + m.getValue())
            ...
            System.getLogger("SonarQubeMetricsReport").log(Logger.Level.ERROR,
                    "Error when trying to execute SonarQube publishing%n " + e.getMessage());
        }
