...
public class MemcachedBulkCacheTest {
  ...
  public void initializeCache() throws IOException {
    ...
    profileCache = new MemcachedBulkCache<>(
        memcachedClient,
        id -> Profile.random(),
        ids -> ids.stream().collect(Collectors.toMap(Function.identity(), id -> Profile.random())),
        "profile",
        Profile.class,
        3600,
        false
    ...
    profileNullableCache = new MemcachedBulkCache<>(
        memcachedClient,
        id -> null,
        ids -> {
    ...
    profileNotNullableCacheWithNull = new MemcachedBulkCache<>(
        memcachedClient,
        id -> null,
        ids -> {
          Map<String, Profile> result = ids.stream().collect(Collectors.toMap(Function.identity(), id -> Profile.random()));
