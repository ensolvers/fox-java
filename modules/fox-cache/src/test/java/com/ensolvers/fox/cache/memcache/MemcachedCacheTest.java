...
class MemcachedCacheTest {
	...
	void testFetchingAndInvalidation() throws InterruptedException {
		...
		}, "testClassCache1", TestClass.class, 3);
	...
	void testAdvancedJacksonSerialization() {
		...
		MemcachedCache<List<TestClass>> cache = new MemcachedCache<>(this.memcachedClient, id -> {
			...
			return Arrays.asList(new TestClass(1L, "someString", 1, 1L), new TestClass(2L, "someString", 2, 2L),
					new TestClass(3L, "someString", 3, 3L));
		}, "testClassCache2", typeFactory -> typeFactory.constructCollectionType(List.class, TestClass.class), 3);
	...
	void testCustomSerialization() {
		...
		}, "testClassCache3", TestClass.class, 3,
