...
public abstract class RedisCache<V> {
  ...
  /**
   * Obtains the final key adding a prefix so a single Redis instance can be shared by several
   * caches.
   *
   ...
   */
  protected String[] collectionOfVToStringArray(Collection<V> values)
  ...
  public void invalidateAll() {
    ...
    do {
      cursor =
          redis.scan(
              ScanArgs.Builder.limit(KEY_SCAN_MAX_LIMIT)
                  .match(this.cacheName + KEY_SEPARATOR + "*"));
  ...
  /**
   * Opens a Redis Transaction, and executes all commands sent within the lambda parameter
   * atomically.
   *
