...
/**
 * Memcached implementation. Instances of this class must be passed to the cache
 * manager.
 */
public class MemcachedCache implements Cache {
     ...
     * @param memcachedClient         the client to do the request to memcached
     *                                service
     * @param expirationTimeInSeconds expiration time of the entries saved in
     *                                memcached
     * @param allowNullValues         if null values are allowed (if true throws a
     *                                {@link CacheInvalidArgumentException} when a
     *                                null value is detected)
     */
    public MemcachedCache(String name, MemcachedClient memcachedClient, int expirationTimeInSeconds, boolean allowNullValues) {
        this.name = name;
    ...
    /**
     * Get operation. Because spring do not provide support for bulk operations we
     * distinguish it checking the return type and the amount and type of parameters
     * For bulk request: return type must be a Map and only one parameter of type
     * Collection must be provided (check {@link CustomCacheKey#isBulk()})
     * 
     * @param key The type of this parameter can be a {@link CustomCacheKey}, List
     *            or a "simple" type (String, Integer, etc)
    ...
    /**
     * Put operation Because spring do not provide support for bulk operations we
     * distinguish it checking the return type and the amount and type of parameters
     * For bulk request: return type must be a Map and only one parameter of type
     * Collection must be provided (check {@link CustomCacheKey#isBulk()})
     * 
    ...
    /**
     * Delete all entries in the cache Note that this will delete all entries in
     * memcached service (global)
     */
    @Override
    public void clear() {
     ...
     * @return the final key (a string conformed with the name of the cache and the
