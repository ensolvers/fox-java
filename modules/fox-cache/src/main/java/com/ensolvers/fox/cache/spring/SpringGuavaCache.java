...
public class SpringGuavaCache<CacheType> extends AbstractValueAdaptingCache {
  ...
  public SpringGuavaCache(
      boolean allowNullValues, String name, long expirationTime, TimeUnit expirationTimeUnit) {
    this(
        allowNullValues,
        name,
        CacheBuilder.newBuilder().expireAfterAccess(expirationTime, expirationTimeUnit).build());
  }
