...
public class SonarQubeService {
    ...
    /**
     * Returns the historic values for a particular metric taken from a specific
     * component within a specific time range
     *
     * @param component key of the component from which the values must be fetched
     * @param metric    specific metric from which we want to get the historic
     *                  values
     ...
     */
    public SonarQubeMetricHistoryResponse getMetricHistory(String component, String metric, Instant from, Instant to)
        ...
        Request request = new Request.Builder()
                .url(SONAR_API_BASE_PATH + "/measures/search_history?" + "from=" + formatter.format(from) + "&to=" + formatter.format(to)
                        + "&component=" + component + "&metrics=" + metric + "&ps=1000")
                .method("GET", null).addHeader("Authorization", "Basic " + tokenInBase64).build();
